...
Widget buildBulletPointList(BuildContext context) {
  final bulletPointsText = context.knobs.string(
    ...
    initialValue:
        'Easy to use and integrate,Fully customizable design,Responsive layout support,Dark mode compatible,Accessibility features included',
  );

  final bulletPoints = bulletPointsText
      .split(',')
      .map((e) => e.trim())
      .toList();

  final showInCard = context.knobs.boolean(
    label: 'Show in Card',
    initialValue: true,
  );

  final showExamples = context.knobs.boolean(
    label: 'Show Examples',
    initialValue: false,
  );
  ...
  return Center(child: DSBulletPointListWidget(bulletPoints: bulletPoints));
}
