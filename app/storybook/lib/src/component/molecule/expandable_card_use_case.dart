...
Widget buildExpandableCard(BuildContext context) {
  final headerText = context.knobs.string(label: 'Header', initialValue: 'Click to expand');
  
  final contentText = context.knobs.string(
    label: 'Content',
    initialValue: 'This is the expanded content that appears when you click the header.',
  );
  
  final showFaqList = context.knobs.boolean(label: 'Show FAQ List', initialValue: false);
  ...
  if (showFaqList) {
    final faqs = [
      ...
      },
      {
        ...
        'answer': 'Storybook is a frontend workshop for building UI components and pages in isolation. It helps you develop and share hard-to-reach states and edge cases.'
      },
    ];
