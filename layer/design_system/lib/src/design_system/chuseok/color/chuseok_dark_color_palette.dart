import 'package:design_system/src/foundation/color/ds_color.dart';
import 'package:design_system/src/foundation/color/ds_color_palette.dart';
import 'package:design_system/src/foundation/color/ds_color_scheme.dart';

/// Chuseok Dark Color Palette
/// Inspired by Korean harvest festival during peaceful autumn evening
/// Celebrates the contemplative moonlit family gatherings and lantern lights
/// Muted traditional colors, warm earth tones, and gentle golds for dark theme
class ChuseokDarkColorPalette implements DSColorPalette {
  const ChuseokDarkColorPalette();

  @override
  BrandColorScheme get brand => const BrandColorScheme(
    // Primary: Moonlight Red - softer traditional red for dark backgrounds
    primary: DSColor(0xFFEF9A9A),
    // Light traditional red
    onPrimary: DSColor(0xFFB71C1C),
    // Dark red text
    primaryContainer: DSColor(0xFFD32F2F),
    // Deep red container
    onPrimaryContainer: DSColor(0xFFFFCDD2),
    // Light red text
    primaryFixed: DSColor(0xFFE53935),
    // Fixed vibrant Korean red
    onPrimaryFixed: DSColor(0xFFFFFFFF),
    primaryFixedDim: DSColor(0xFFD32F2F),
    onPrimaryFixedVariant: DSColor(0xFFB71C1C),

    // Secondary: Lantern Gold - warm evening harvest colors
    secondary: DSColor(0xFFFFCC02),
    // Bright lantern gold
    onSecondary: DSColor(0xFFE65100),
    // Dark orange text
    secondaryContainer: DSColor(0xFFFF8F00),
    // Rich harvest gold container
    onSecondaryContainer: DSColor(0xFFFFE0B2),
    // Light golden text
    secondaryFixed: DSColor(0xFFFFA000),
    // Fixed bright harvest gold
    onSecondaryFixed: DSColor(0xFF000000),
    secondaryFixedDim: DSColor(0xFFFF8F00),
    onSecondaryFixedVariant: DSColor(0xFFE65100),

    // Tertiary: Evening Blue - calm traditional blue for night
    tertiary: DSColor(0xFF64B5F6),
    // Light traditional blue
    onTertiary: DSColor(0xFF0D47A1),
    // Dark blue text
    tertiaryContainer: DSColor(0xFF1976D2),
    // Traditional blue container
    onTertiaryContainer: DSColor(0xFFBBDEFB),
    // Light blue text
    tertiaryFixed: DSColor(0xFF2196F3),
    // Fixed bright Korean blue
    onTertiaryFixed: DSColor(0xFFFFFFFF),
    tertiaryFixedDim: DSColor(0xFF1976D2),
    onTertiaryFixedVariant: DSColor(0xFF0D47A1),

    inversePrimary: DSColor(0xFFD32F2F),
  );

  @override
  SemanticColorScheme get semantic => const SemanticColorScheme(
    // Error: Gentle autumn red
    error: DSColor(0xFFEF5350),
    onError: DSColor(0xFFB71C1C),
    errorContainer: DSColor(0xFFD32F2F),
    onErrorContainer: DSColor(0xFFFFCDD2),

    // Warning: Warm persimmon
    warning: DSColor(0xFFFFB74D),
    onWarning: DSColor(0xFFE65100),
    warningContainer: DSColor(0xFFFF6F00),
    onWarningContainer: DSColor(0xFFFFE0B2),

    // Success: Moonlit pine
    success: DSColor(0xFF66BB6A),
    onSuccess: DSColor(0xFF1B5E20),
    successContainer: DSColor(0xFF2E7D32),
    onSuccessContainer: DSColor(0xFFC8E6C9),

    // Info: Evening sky blue
    info: DSColor(0xFF42A5F5),
    onInfo: DSColor(0xFF0D47A1),
    infoContainer: DSColor(0xFF1976D2),
    onInfoContainer: DSColor(0xFFBBDEFB),
  );

  @override
  NeutralColorScheme get neutral => const NeutralColorScheme(
    white: DSColor(0xFF1A1612),
    // Very dark warm background
    grey1: DSColor(0xFF211E1A),
    // Dark surface with warm hint
    grey2: DSColor(0xFF2A2722),
    // Elevated warm surface
    grey3: DSColor(0xFF3A352E),
    // Medium dark warm surface
    grey4: DSColor(0xFF4A453E),
    // Lighter dark warm surface
    grey5: DSColor(0xFF616161),
    // Mid grey
    grey6: DSColor(0xFF757575),
    // Light medium grey
    grey7: DSColor(0xFF9E9E9E),
    // Light grey
    grey8: DSColor(0xFFBDBDBD),
    // Very light grey
    grey9: DSColor(0xFFE0E0E0),
    // Near white
    grey10: DSColor(0xFFF5F5F5),
    // Almost white
    black: DSColor(0xFFFFFFFF), // White text on dark
  );

  @override
  SurfaceColorScheme get surface => const SurfaceColorScheme(
    surface: DSColor(0xFF1A1612),
    // Deep warm dark background
    onSurface: DSColor(0xFFE6DDD7),
    // Light warm text
    surfaceVariant: DSColor(0xFF2A1F0F),
    // Very dark warm tint
    onSurfaceVariant: DSColor(0xFFEF9A9A),
    // Light red text
    surfaceDim: DSColor(0xFF16130F),
    // Darker warm surface
    surfaceBright: DSColor(0xFF2A2722),
    // Brighter dark surface
    surfaceContainerLowest: DSColor(0xFF0F0F0F),
    // Deepest container
    surfaceContainerLow: DSColor(0xFF1A1A1A),
    // Low elevation
    surfaceContainer: DSColor(0xFF242424),
    // Standard container
    surfaceContainerHigh: DSColor(0xFF2E2E2E),
    // High elevation
    surfaceContainerHighest: DSColor(0xFF383838),
    // Highest elevation
    inverseSurface: DSColor(0xFFE6DDD7),
    // Light inverse
    onInverseSurface: DSColor(0xFF313033),
    // Dark text on light
    inverseOnSurface: DSColor(0xFFF4EFF4), // Light text
  );

  @override
  OutlineColorScheme get outline => const OutlineColorScheme(
    outline: DSColor(0xFF938F99), // Lighter neutral outline for dark
    outlineVariant: DSColor(0xFF49454F), // Dark outline variant
  );

  @override
  UtilityColorScheme get utility => const UtilityColorScheme(
    shadow: DSColor(0xFF000000),
    scrim: DSColor(0xFF000000),
    surfaceTint: DSColor(0xFFEF9A9A), // Light red tint for dark
  );
}
